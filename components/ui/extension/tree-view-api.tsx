...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      ref={ref}
      className={cn(
        "h-full w-px bg-muted absolute left-1.5 rtl:right-1.5 py-3 rounded-md hover:bg-slate-600 duration-300 ease-in-out",
        className,
      )}
      {...props}
    />
      ...
      >
        ...
        <AccordionPrimitive.Content className="text-sm data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down relative overflow-hidden h-full">
          ...
          <AccordionPrimitive.Root
            dir={direction}
            type="multiple"
            className="flex justify-start flex-col gap-1 py-1 ml-5 rtl:mr-5 "
            defaultValue={expendedItems}
            value={expendedItems}
            onValueChange={(value) => {
              setExpendedItems?.((prev) => [...(prev ?? []), value[0]]);
